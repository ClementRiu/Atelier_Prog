...
void Joueur::tourGestion(CarteduMonde &carte, Bouton boutonFinTour, Bouton menu,
                         Bouton boutonAction, Bouton boutonInventaire, bool &quit, std::vector<Unite*> unite1, std::vector<Unite*> unite2) {
    Unite *unite;
    ...
    while (true) {
        ...
        if (dans_mmap(x, y)) {
            ...
            Imagine::Coords<2> pos_clique = translate(x, y, pos_centrelu[0], pos_centrelu[1]);
            Imagine::flushEvents();
            carte.deplace_souris(pos_clique, unite1, unite2);
        }
        ...
        if (menu.boutonActive(x, y)) {
            int choix = affiche_menu();
            /*sauvegarde(herosJoueur);
            quit = true;
            break;*/
        }
        x = x - widthworld  + carte.get_pos_centre()[0] * taille_case_graph;
        y = y - heightworld + carte.get_pos_centre()[1] * taille_case_graph;
        std::cerr<<x<<"  "<<y<<std::endl;
        // Vient-on de cliquer sur une unite ?
        if (numeroCase(x, y, carte.get_pos_centre()) != -1 && carte[numeroCase(x, y, carte.get_pos_centre())].getOccupe()) {
            unite = carte[numeroCase(x, y, carte.get_pos_centre())].getUnite();
            ...
            if (boutonInventaire.boutonActive(x, y)) {
                ...
                if ((abs(unite->getCase() - numeroCase(x, y, carte.get_pos_centre())) == 1 ||
                     abs(unite->getCase() - numeroCase(x, y, carte.get_pos_centre())) == NbCase) && numeroCase(x, y, carte.get_pos_centre()) != -1) {
                    //On déclanche un évènement (ex : ouvrir une ville) si on a affaire à une case spéciale
                    carte[numeroCase(x, y, carte.get_pos_centre())].action(unite);
                    carte.affiche(unite1, unite2);

                    //S'il y a un héros, on déclanche un combat
                    if (carte[numeroCase(x, y, carte.get_pos_centre())].getOccupe()) {
                        Unite *uniteCliquee = carte[numeroCase(x, y, carte.get_pos_centre())].getUnite();
