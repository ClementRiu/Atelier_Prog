...
void afficheChemins(int x, int y, CarteduMonde &carte, std::vector<std::vector<int> > differentsChemins, int numcase) {
    if (numeroCase(x, y, carte.get_pos_centre()) != -1) {
        // L'entier suivant correspont à la case la plus proche de (x,y) qui est en brillance. == -1 si aucune.
        int caseProche = carte[numeroCase(x, y, carte.get_pos_centre())].plusProcheVoisineBrillante(x, y, carte, numeroCase(x, y, carte.get_pos_centre()));
        int num;
        // La dernière condition signifie que l'on est pas en train de regarder la position du Heros
        if (carte[numeroCase(x, y, carte.get_pos_centre())].Brillance() ||
            (carte[numeroCase(x, y, carte.get_pos_centre())].getOccupe() && caseProche != -1 &&
             numeroCase(x, y, carte.get_pos_centre()) != numcase)) {
            // On met dans num le numéro de la case où l'on veut aller
            if (carte[numeroCase(x, y, carte.get_pos_centre())].Brillance()) {
                num = numeroCase(x, y, carte.get_pos_centre());
            }
            else {
                num = caseProche;
            }
            ...
            for (int i = 0; i < differentsChemins.size(); ++i) {
                if (differentsChemins[i][differentsChemins[i].size() - 1] == num) {
                    for (int j = 0; j < differentsChemins[i].size(); ++j) {
                        ...
                        carte[differentsChemins[i][j]].affiche_chemin();
                    }
                }
            }
        }
